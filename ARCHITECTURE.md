...
- Graph-first operating shell: mode bars, command palette, progressive control drawer, floating graph toolbars, contextual inspectors, stock minimap, graph breadcrumbs, and mobile bottom sheets. Persistent chrome should assist the graph, not compete with it.
...
- Contextual inspector: selected company, relationship, evidence, Crypto wallet, and Crypto flow details appear as transient right-side inspectors on desktop and bottom sheets on mobile. The graph remains the primary surface when no object is being inspected.
