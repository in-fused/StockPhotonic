...
Large-graph readability now treats `mega` density, semantic zoom, and cinematic state as first-class states. Rendering and scaling helpers throttle low-priority labels and edges by viewport, density, route/selection priority, source evidence, SEC-backed status, overlay state, guided navigation state, and mode-sensitive detail rules. Selected neighborhoods, active routes, strategic hubs, corridor lanes, and reviewer/replay focus paths override broad throttling. Layout helpers and D153 screen-space focus offsets increase breathing room for dense stock clusters, strategic corridors, and Crypto liquidity/wallet topology while preserving the same graph engine and static data contracts.
